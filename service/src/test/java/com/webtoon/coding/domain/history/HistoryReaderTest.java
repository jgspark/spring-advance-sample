...
class HistoryReaderTest {
    ...
    public class GetAll {
        ...
        public void testGetAll() {
            ...
            when(historyRepository.findAllProjectedBy(any(), eq(HistoryInfo.class))).thenReturn(mocks);
            ...
            assertArrayEquals(mocks.getContent().toArray(new HistoryInfo[0]),
                    entity.getContent().toArray(new HistoryInfo[0]));
        }
        ...
        public void testGetAllFail() {
            RuntimeException e = assertThrows(RuntimeException.class,
                    () -> historyCustomReader.getAll(PageRequest.of(0, 10), null));
    ...
    public class GetHistories {
        ...
        public void testGetHistories() {
            ...
            Page<HistoryUser> mocks = HistoryMock.createPageHistoryUser(UserMock.createdMock(),
                    ContentsMock.createdMock());

            when(historyRepository.findByCreatedDateBetweenAndContents_AdultType(any(), any(), any(), any(), anyLong()))
                .thenReturn(mocks);

            PageHistoryUser dto = PageHistoryUser.of(PageRequest.of(0, 10), new Date(), new Date(), Adult.NONE_ADULT,
                    3L);

            Page<HistoryUser> entity = historyCustomReader.getHistories(dto);

            verify(historyRepository, times(1)).findByCreatedDateBetweenAndContents_AdultType(any(), any(), any(),
                    any(), anyLong());

            assertArrayEquals(entity.getContent().toArray(new HistoryUser[0]),
